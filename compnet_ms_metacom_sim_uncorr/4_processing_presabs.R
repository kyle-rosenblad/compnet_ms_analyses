...
for(i in 1:length(nmpa_nulls)){
  r <- readRDS(paste("nmpa_results_", nmpa_nulls[i], "_presabs.RDS", sep=""))
  r <- as.data.frame(r)
  names(r) <- paste(names(r), nmpa_nulls[i], sep="_")
  param_levels <- cbind(param_levels, r)
}
for(i in 1:length(nmpa_nulls)){
  r <- readRDS(paste("nmpa_results_", nmpa_nulls[i], "_presabs_neg.RDS", sep=""))
  r <- as.data.frame(r)
...
saveRDS(methods, "../compnet_ms_metacom_sim_summary/summary_uncorr_presabs.RDS")
saveRDS(param_levels, "../compnet_ms_metacom_sim_summary/pl_uncorr_pa.RDS")
