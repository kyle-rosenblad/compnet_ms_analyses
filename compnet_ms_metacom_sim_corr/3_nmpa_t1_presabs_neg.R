...
nmpa_res <- foreach(g=1:nrow(param_levels), .combine="rbind") %dopar% {
  ...
  if(coocc_sites>0 & rich>=3){
    ...
    reshuffle_traits <- function(indf) {
      indf$ndtrait <- sample(indf$ndtrait)
      return(indf)
    }
    
    randtraits <- lapply(1:999, function(x) reshuffle_traits(sp_df2))
...
saveRDS(nmpa_res, "nmpa_results_t1_presabs_neg.RDS")
