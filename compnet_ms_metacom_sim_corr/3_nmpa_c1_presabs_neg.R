...
nmpa_res <- foreach(g=1:nrow(param_levels), .combine="rbind") %dopar% {
  ...
  if(coocc_sites>0 & rich>=3){
    ...
    reshuffle_rows <- function(inmat) {
      t(apply(inmat, 1, reshuffle_nonzero))
    }
    fix_column_names <- function(inmat){
      colnames(inmat) <- colnames(mat)
      inmat
    }
    randmats <- lapply(1:999, function(x) reshuffle_rows(mat))
    randmats <- lapply(randmats, fix_column_names)
...
saveRDS(nmpa_res, "nmpa_results_c1_presabs_neg.RDS")
